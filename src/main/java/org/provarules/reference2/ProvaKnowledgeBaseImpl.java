...
public class ProvaKnowledgeBaseImpl implements ProvaKnowledgeBase {
    ...
    public ProvaKnowledgeBaseImpl() {
        ...
        // FHIR Integration
        //System.out.println("Including the fhir_query_data - builtin");
        builtins.put("fhir_connect", new FhirConnectImpl(this));
        builtins.put("fhir_query_and_prepare", new FhirQueryAndPrepareImpl(this));
        builtins.put("fhir_native_query", new FhirNativeQueryImpl(this));
        builtins.put("printFhirObservation", new FhirPrintObservationResourceContentImpl(this));
        builtins.put("fhirExtractValueFromObservation", new FhirExtractValueFromObservationImpl(this));
        builtins.put("fhirCompareBigDecimalValues", new FhirCompareBigDecimalValues(this));
        initRules();
...
//				"rcvMsg(XID,Protocol,From,Verb,Payload,CorrelationID):-'@temporal_rule'(XID,TID,[XID,Protocol,From,Verb,Payload,CorrelationID]).\n" +
    ...
    public /*synchronized*/ ProvaPredicate getOrGeneratePredicate(String symbol, int arity) {
		...
		*/
        ...
        if (predicate == null) {
            predicate = new ProvaPredicateImpl(symbol, arity, this);
            //System.out.println("Adding a new predicate " + predicate.getSymbol() + " key: " + key);
            predicates.put(key, predicate);
        }
		/*
		key = symbol+"/-1";
